...
class PorkchopCalculator {
    ...
    public computeAllDeltaVs() {
        ...
        for(let i=0; i<this._nTimes; i++) {
            ...
            self.postMessage({deltaVs: [], index: i, finished: false})
        }
...
self.onmessage = (event: MessageEvent<{inputs: PorkchopInputs}>) => {
    const calculator = new PorkchopCalculator(event.data.inputs);
    calculator.computeAllDeltaVs();
    const data = calculator.data;
    self.postMessage({deltaVs: data.deltaVs, index: 0, finished: true})    
}